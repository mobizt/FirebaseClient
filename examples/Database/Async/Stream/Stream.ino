...
void printResult(AsyncResult &aResult)
{
    ...
    if (aResult.available())
    {
        ...
        {
            Serial.printf("event: %s\n", aResult.database.event().c_str());
            Serial.printf("path: %s\n", aResult.database.dataPath().c_str());
            Serial.printf("data: %s\n", aResult.database.to<const char *>());
            Serial.printf("type: %d\n", aResult.database.type());

            // The stream event from result.database.stream can be converted to the the values.
            bool v1 = aResult.database.to<bool>();
            int v2 = aResult.database.to<int>();
            float v3 = aResult.database.to<float>();
            double v4 = aResult.database.to<double>();
            String v5 = aResult.database.to<String>();
        }
        else
        {
