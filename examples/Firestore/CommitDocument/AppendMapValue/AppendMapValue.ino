 ...
 * SYNTAXES:
 *
 * firestore.commitDocument(<AsyncClient>, <ParentResource>, <Writes>);
 * firestore.commitDocument(<AsyncClient>, <ParentResource>, <Writes>, <AsyncResult>);
 * firestore.commitDocument(<AsyncClient>, <ParentResource>, <Writes>, <AsyncResultCallback>, <uid>);
 *
...
void loop()
{
    ...
    if (app.ready() && (millis() - dataMillis > 60000 || dataMillis == 0))
    {
        ...
        firestore.commitDocument(aClient, ParentResource(FIREBASE_PROJECT_ID), writes, asyncCB);

        // To assign UID for async result
        // firestore.patchDocument(aClient, ParentResource(FIREBASE_PROJECT_ID), documentPath, DocumentMask("myMap.key" + String(counter)), DocumentMask(), doc, Precondition(), asyncCB, "myUID");

        // To get anyc result without callback
        // firestore.patchDocument(aClient, ParentResource(FIREBASE_PROJECT_ID), documentPath, DocumentMask("myMap.key" + String(counter)), DocumentMask(), doc, Precondition(), aResult_no_callback);
    }
}
