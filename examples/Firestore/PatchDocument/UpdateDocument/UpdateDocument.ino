...
void loop()
{
    ...
    if (app.ready() && (millis() - dataMillis > 60000 || dataMillis == 0))
    {
        ...
        // firestore.patchDocument(aClient, ParentResource(FIREBASE_PROJECT_ID), documentPath, DocumentMask("count,status"), DocumentMask(), doc, Precondition(), asyncCB, "myUID");

        // To get anyc result without callback
        // firestore.patchDocument(aClient, ParentResource(FIREBASE_PROJECT_ID), documentPath, DocumentMask("count,status"), DocumentMask(), doc, Precondition(), aResult_no_callback);
    }
}
