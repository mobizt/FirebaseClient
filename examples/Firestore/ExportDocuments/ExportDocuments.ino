 ...
 * SYNTAXES:
 *
 * firestore.exportDocuments(<AsyncClient>, <ParentResource>, <collectionIds>, <bucketID>, <storagePath>);
 * firestore.exportDocuments(<AsyncClient>, <ParentResource>, <collectionIds>, <bucketID>, <storagePath>, <AsyncResult>);
 * firestore.exportDocuments(<AsyncClient>, <ParentResource>, <collectionIds>, <bucketID>, <storagePath>, <AsyncResultCallback>, <uid>);
 *
...
void loop()
{
    ...
    if (app.ready() && !taskCompleted)
    {
        ...
        firestore.exportDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID, "" /* databaseId can be (default) or empty */), "" /* Which collection ids to export. Unspecified means all collections. */, STORAGE_BUCKET_ID, "test_path" /* The path in the Firebase Storage bucket to store the data */, asyncCB);
