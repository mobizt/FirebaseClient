...
/**
 * LIST DOCUMENTs FUNCTIONS
 * ========================
 *
 * SYNTAXES:
 *
 * firestore.listDocuments(<AsyncClient>, <ParentResource>, <documentPath>, <ListDocumentsOptions>);
 * firestore.listDocuments(<AsyncClient>, <ParentResource>, <documentPath>, <ListDocumentsOptions>, <AsyncResult>);
 * firestore.listDocuments(<AsyncClient>, <ParentResource>, <documentPath>, <ListDocumentsOptions>, <AsyncResultCallback>, <uid>);
 *
...
void loop()
{
    ...
    if (app.ready() && (millis() - dataMillis > 60000 || dataMillis == 0))
    {
        ...
        if (!taskCompleted)
        {
            ...
            firestore.listDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), collectionId, listDocsOptions, asyncCB);

            // To assign UID for async result
            // firestore.listDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), collectionId, listDocsOptions, asyncCB, "myUID");

            // To get anyc result without callback
            // firestore.listDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), collectionId, listDocsOptions, aResult_no_callback);
        }
    }
}
