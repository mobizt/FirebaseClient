...
void loop()
{
    ...
    if (app.ready() && !taskCompleted)
    {
        ...
        firestore.importDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), importOptions, asyncCB);

        // To assign UID for async result
        // firestore.importDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), importOptions, asyncCB, "myUID");

        // To get anyc result without callback
        // firestore.importDocuments(aClient, ParentResource(FIREBASE_PROJECT_ID), importOptions, aResult_no_callback); }
    }
}
